...
export class VerifyTransactionContext {
  ...
  validate(compatible = false) {
    ...
    new BytesVec(this.view.buffer.slice(offsets[2], offsets[3]), {
      validate: false,
    }).validate();
    new ScriptVec(this.view.buffer.slice(offsets[3], offsets[4]), {
      validate: false,
    }).validate();
    new Byte32(this.view.buffer.slice(offsets[4], offsets[5]), {
      validate: false,
    }).validate();
    new BlockHashEntryVec(this.view.buffer.slice(offsets[5], offsets[6]), {
      validate: false,
    }).validate();
  }
