...
import { PolyjuiceWallet, PolyjuiceWebsocketProvider } from "../lib/index";
...
var provider: PolyjuiceWebsocketProvider | ethers.providers.WebSocketProvider;
...
test.before((t) => {
  ...
  const web3_http_rpc = process.env.WEB3_JSON_RPC;
  const web3_ws_rpc = process.env.WEB3_WS_JSON_RPC;
  const polyjuiceConfig: PolyjuiceConfig = {
    abiItems: SimpleStorageV2_Abi as AbiItems,
    web3Url: web3_http_rpc,
  };
  provider = new PolyjuiceWebsocketProvider(polyjuiceConfig, web3_ws_rpc);
  //provider = new ethers.providers.WebSocketProvider(web3_ws_rpc);
